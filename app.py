...
db = None
sheet = None
GEMINI_API_KEY = os.environ.get('GEMINI_API_KEY') # Render 환경 변수에서 Gemini API 키 불러오기
...
def generate_question_from_ai():
    ...
    **출력 형식은 반드시 아래의 JSON 스키마를 따라야 해:**
    ...
    try:
        ...
        # JSON 형식의 텍스트를 파싱
        # Gemini가 코드 블록(```json ... ```)으로 응답하는 경우가 있어 제거 처리
...
def get_test():
    ...
    try:
        # Firestore에서 모든 문제를 가져와서 랜덤으로 7개 선택
        questions_ref = db.collection('questions').stream()
        all_questions = [doc.to_dict() for doc in questions_ref]
        
        if len(all_questions) < 7:
            return jsonify(all_questions) # 7개 미만이면 모두 반환
        
        return jsonify(random.sample(all_questions, 7))
    except Exception as e:
        print(f"문제 불러오기 오류: {e}")
        return jsonify([]), 500

# ... (generate-code, get-codes, validate-code, submit-result 등 생략)
